...
**DAG (causal)**: Directed graph of causal relationships; tells you what to adjust for and, just as important, what not to.
...
**LoRA**: Low-rank adapters trained while the base model stays frozen. Works because weight *updates* are low-rank. See [Fine-Tuning](../deep_learning/intro_fine_tuning.md).
...
**Shadow deployment**: New model scores real traffic with output discarded. The only validation step with zero user risk.
