...
**BatchNorm's train/eval trap**: at training time it uses batch statistics; at eval it uses running averages. Forgetting `model.eval()` at inference means predictions depend on whatever else is in the batch: a common production bug that also breaks batch-size-1 serving.
...
| **Data augmentation** | Expands the effective dataset | Usually the most effective option |
