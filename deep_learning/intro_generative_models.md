...
A crucial property is that you can jump to any timestep in closed form, which is what makes training tractable — no need to simulate the chain:
...
FID embeds real and generated images with an Inception network and compares the two feature distributions as Gaussians via Fréchet distance. The crucial property is that it compares **distributions rather than individual samples**, so it penalizes mode collapse — a GAN producing ten perfect images scores terribly, which per-sample quality measures would miss.
